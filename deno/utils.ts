import { HSVToRGB, StringToRGB } from './ColorConverters.ts';
import { RGBColor, HSVColor, StringColor } from './ColorTypes.ts';
...
export function convertCssColorToHex(color: string): string {
  if (cssColors[color.toLowerCase()] !== undefined) return cssColors[color.toLowerCase()];
  return color;
}
