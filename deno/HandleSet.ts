import { isValidStringColor } from './validators.ts';
import { RGBColor } from './ColorTypes.ts';

export function ConvertString(input: string, return255?: boolean, alpha255?: boolean) {
...
function HandleConvertRgbString(text: string, return255?: boolean) {
  ...
  return new RGBColor(
    parseInt(split[0].replace(/\D/g, '')) / (return255 ? 1 : 255),
    parseInt(split[1].replace(/\D/g, '')) / (return255 ? 1 : 255),
    parseInt(split[2].replace(/\D/g, '')) / (return255 ? 1 : 255),
